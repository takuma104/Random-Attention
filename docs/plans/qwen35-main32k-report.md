...
**以下は自動採点器のスコア。既知の表記差によるfalse negativeを含む。** [別versionの感度分析](qwen35-grading-sensitivity-report.md)では56回答を補正し、native94.7%、Random1024 72.4%、Random2048 82.4%、Recency69.3%、SnapKV69.4%。主要な結論は変わらない。主評価JSONを書き換えたり、途中で採点規則を変更したりしていない。
...
- EOS不正解判定は全条件合計66回答。全件の表記差レビューで56例を補正。残る10例は解釈未解決5例・final boxなし3例・確認できる計算誤り2例。これはAI-assistedの事後レビューで、独立した人間による全件採点ではない。
